...
export default function RegisterPage() {
  ...
  // Credentials handed over from the login page's "Create an account" link
  ...
  // Maps DRF's { field: [messages] } error body onto our field-error state
  const applyApiErrors = (data) => {
    if (!data || typeof data !== 'object') return false;
    const mapped = {};
    if (data.full_name) mapped.name = [].concat(data.full_name)[0];
    if (data.email) mapped.email = [].concat(data.email)[0];
    if (data.password) mapped.password = [].concat(data.password)[0];
    if (data.confirm_password)
      mapped.confirmPassword = [].concat(data.confirm_password)[0];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!applyApiErrors(data)) {
        setApiError(
          data?.detail || error.message || 'Registration failed. Please try again.'
        );
      }
    } finally {
      setIsLoading(false);
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl text-center">
          ...
          <p className="text-sm text-slate-400 mt-2">
            ...
            <span className="text-slate-200 font-medium">{email}</span>. Click it
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 selection:bg-blue-500 selection:text-white">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl transition-all duration-300">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-sm text-slate-400 mt-1">Join DevLaunch to launch your sites</p>
        </div>

        {apiError && (
          <div className="mb-6 p-4 bg-rose-950/40 border border-rose-500/30 rounded-xl text-rose-300 flex items-start gap-3">
